...
{
  reco::MuonPFIsolation isoBlock = muon.pfIsolationR04();
  std::vector<reco::PFCandidatePtr> tauChargedCands = (*tau).signalPFChargedHadrCands(); //reco::PFCandidateRefVector tauChargedCands = (*tau).signalPFChargedHadrCands();
  std::vector<reco::PFCandidatePtr> tauNeutralCands = (*tau).signalPFNeutrHadrCands(); //reco::PFCandidateRefVector tauNeutralCands = (*tau).signalPFNeutrHadrCands();
  std::vector<reco::PFCandidatePtr> tauGammaCands = (*tau).signalPFGammaCands(); //reco::PFCandidateRefVector tauGammaCands = (*tau).signalPFGammaCands();

  double tauChargedHadronPt = 0.;
  double tauNeutralHadronPt = 0.;
  double tauPhotonPt = 0.;

  for (/*reco::PFCandidateRefVector::const_iterator*/ std::vector<reco::PFCandidatePtr>::const_iterator iCand = tauChargedCands.begin(); iCand != tauChargedCands.end(); ++iCand)
    {
      if (deltaR(muon.p4(), (*iCand)->p4()) < 0.4)
	tauChargedHadronPt += (*iCand)->pt();
    }

  for (/*reco::PFCandidateRefVector::const_iterator*/ std::vector<reco::PFCandidatePtr>::const_iterator  iCand = tauNeutralCands.begin(); iCand != tauNeutralCands.end(); ++iCand)
    {
      if (deltaR(muon.p4(), (*iCand)->p4()) < 0.4)
	tauNeutralHadronPt += (*iCand)->pt();
    }

  for (/*reco::PFCandidateRefVector::const_iterator*/ std::vector<reco::PFCandidatePtr>::const_iterator  iCand = tauGammaCands.begin(); iCand != tauGammaCands.end(); ++iCand)
    {
      if (deltaR(muon.p4(), (*iCand)->p4()) < 0.4)
