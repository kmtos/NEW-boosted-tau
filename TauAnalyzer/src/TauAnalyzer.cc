...
//       }
    ...
    const std::vector<reco::PFCandidatePtr> tauSigCands((*iTau)->signalPFCands());
...
//       std::cerr << tauOldJetRef->eta() << " " << tauOldJetRef->phi() << std::endl;
	...
	dRSoftMuNearestGenZOrTTMuFSR = reco::deltaR(*removedMuonRefs[removedMuonRefs.size() - 1], 
      ...
      const std::vector<reco::PFCandidatePtr> tauHadSigChargedHadrons = (*iTau)->signalPFChargedHadrCands();
      const std::vector<reco::PFCandidatePtr> tauHadSigNeutralHadrons = (*iTau)->signalPFNeutrHadrCands();
      for (std::vector<reco::PFCandidatePtr>::const_iterator iTauHadSigChargedHadron = tauHadSigChargedHadrons.begin(); iTauHadSigChargedHadron != tauHadSigChargedHadrons.end(); ++iTauHadSigChargedHadron) {
	tauVecExcludingPhotons+=(*iTauHadSigChargedHadron)->p4();
      }
      for (std::vector<reco::PFCandidatePtr>::const_iterator 
	...
	the other hadronic tau constituents*/
      const std::vector<reco::PFCandidatePtr>& tauHadSigPhotons = (*iTau)->signalPFGammaCands();
      double tauHadPhotonEnergy = 0.0;
      for (std::vector<reco::PFCandidatePtr>::const_iterator iTauHadSigPhoton = tauHadSigPhotons.begin(); 
			   ...
			   Float_t Etamin, Float_t Etamax, TString DataPeriod, double ptEval)
{
  ...
  if( Atagger == "CSVM" && sEtamin == "0.0" && sEtamax == "2.4")
      ...
      if( meanminmax == "min" )
	{
	  ...
	  unsigned int bErrorBin = 100000000; //Kyle was getting error that this can't have the possibility of returning uninitialized. So I initialized it to be massive
